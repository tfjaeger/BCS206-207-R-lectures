...
d.rucci = read_csv("./data/Rucci/Michele Good Data 15-12-00.csv") %>%
  arrange(session_idx, Unity_timestamp)

summary(d.rucci)

d.rucci %>%
  filter(session_idx > 0) %>%
  View()

d.rucci %<>%
  left_join(
    d.rucci %>%
      group_by(session_idx) %>%
...
d.rucci %>%
  mutate(stimulus_deg_delta = factor(stimulus_deg_delta)) %>%
  summary()
```

## Plotting the data

### Plotting left and right eye's x coordinate by stimulus degree
...
d.rucci %>%
  ...
  facet_wrap(~stimulus_deg_delta)
```

### Plotting left and right eye's x and y coordinates by stimulus degree
