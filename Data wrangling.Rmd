...
## Dplyr's verbs
Dplyr has 'verbs' like filter, select, summarize, mutate, transmute, etc. to let use conduct operations on our data, and reshape the data frame into the format we need. Consider the following data.frame from an experiment with a 2AFC task in three within-subject conditions (A, B, C):
...
We can calculate the proportion correct by using summarise. 
 ...
 * How would you calculate the by-subject averages for all three conditions?
 * How could you also calculate the by-subject standard deviations around those averages?
 * How could you attach this information (the averages and SDs) to each row of the present data.frame?

# An example: Haefner group

This group seeks to replicate [Herce Castanon et al. (2019)](https://www.nature.com/articles/s41467-019-09330-7).

![Figure 2 from Herce Castanon et al. (2019)](./figures/HerceCastanonFigure2.png)
...
p1 = d.haefner %>%
  ...
  stat_summary(fun.y = mean, geom="bar", position=p, fill = "blue", color = NA) +
  stat_summary(fun.data = mean_cl_boot, geom = "linerange", position=p) +
  scale_x_discrete("") +
  scale_y_continuous("Proportion correct") +
