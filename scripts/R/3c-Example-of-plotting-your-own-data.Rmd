---
title: 'Data Wrangling and Visualization 101 in R - Going through example data sets'
...
d.speech.exposure %>%
  ...
  summarise_at(
    ...
    mean) %>% 
  # For some subjects, RTs were not recorded. I'm setting the RTs to the average of all other subject. 
  # (for the current purpose this is ok since we simply don't know their RTs and thus would not exclude 
  # them based on their RTs)
  mutate(AdjustedRT = ifelse(is.na(AdjustedRT), mean(AdjustedRT, na.rm = T), AdjustedRT)) %>%
  ggplot(
    aes(x = AdjustedRT, y = IsCorrect)) +
  # geom_label and geom_text take the aesthetic label to plot text. geom_label additionally puts a box 
  # around the text, and that makes things a little easier to read.
  geom_label(
    aes(label = Subject),
    alpha = .5) +
  # I'm transforming the coordinate system along the x-axis to use a log-axis. Note that the data (and 
  # thus also the statistics, such as obtaining the subject-specific mean) are still done in terms of 
  # raw RTs. That means that a single (or a few) extreme values can change the mean RTs of the subject 
  # quite a bit. That's something to think about when you think about exclusion criteria.
  coord_trans(
    x = "log10")
```
...
```{r}
d.speech.test %>%
  # Removing the control condition
  filter(Label != "FILLER") %>%
  # Obtaining subject-level means for each label condition and continuum step
  ...
  # pointranges and lines to summarize the distribution of by-subject means
  stat_summary(fun.data = mean_cl_boot, geom = "pointrange") +
  stat_summary(fun.data = mean_cl_boot, geom = "line") +
  # remapping the values of step onto a scale from 1 to 6.
  scale_x_continuous(
    "Continuum step",
    breaks = sort(unique(d.speech.test$Step)),
    labels = 1:6
  ) +
  # remapping the values of the label condition
