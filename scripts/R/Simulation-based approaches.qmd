  ...
  summarise(
    across(
      ...
      list(
        ...
        "Type_I_error" = ~ round(mean(.x < .05, na.rm = T), 3)),
    .names = "{.fn}_{.col}")) %>%
  rename_all(.funs = ~ gsub("p\\.value_", "", .x))
```
...
- We can compare the Type I error rate of the $t$-test obtained above to the Type I error rate of the same $t$-test if the data is generated following the ground truth (see [Git repo](https://github.com/tfjaeger/BCS206-207-R-lectures/blob/2021-2022/scripts/R/6-Type-I-error-and-power-analyses.Rmd)).
