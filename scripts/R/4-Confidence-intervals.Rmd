...
To illustrate this with an example, recall the experiment on visual crowding introduced in a [previous lecture](https://github.com/tfjaeger/BCS206-207-R-lectures/blob/2024-PhD-skillz-workshop/scripts/R/3a-Data-wrangling-and-visualization.pdf). To recap that experiment briefly:
...
where $SD_y$ is the *sample's* standard deviation of y---i.e., the standard deviation of $y$ in our observed sample. Why is it $\pm 1.96$? Because 95% of the density of a normal distribution falls between its mean $\pm 1.959964...$ its standard deviation---i.e., between its 2.5th and 97.5th quantile. E.g. for our example from above, a normal distribution with mean 10 and standard deviation 2:
...
To make this more concrete, consider the experiment on visual crowding described above. Let's calculate the 95% CIs for the six conditions defined by crossing the three number sizes and two crowding conditions. We'll do so for subject 1.
...
## Non-parametric confidence intervals from a sample-based approach

One non-parametric method to calculate CIs is the **bootstrap**. The bootstrap (Efron & Tibshirani, 1994) is a highly powerful, general-purpose sampling-based approach. It can be used for many purposes, and here we use it to construct confidence intervals. There are many different implementations of the bootstrap, and some of the details can become quite complicated depending on the type of data you are analyzing. At the core though, the bootstrap is based on a rather intuitive idea. Rather than to make parametric assumptions about the distribution of the data and the resulting expected distribution of the sample means (across samples), we can use the sample data we have to estimate the distribution. Specifically, we can randomly sample with replacement a new sample *from the sample data we have*. For example, for subject 1 from the same experiment as above we resample 720 new trials (balanced in the same way as before) from the 720 trials we have from that subject:
...
Note that these are not the same trials as before. We can see that by plotting a histogram of the original trial ID in the newly resampled data. We can see that some trial IDs (x-axis) were sampled multiple times (see the counts on the y-axis), and others were never sampled:
...
Now we simply ask what the 2.5th to 97.5th quantiles of each of these six distribution of means are. Basically, we determine the range symmetric around the mean in which 95% of the resampled means fall. Notice that the CIs based on these quantiles do not (and can never) fall outside of the possible range of the data. Notice also that the CIs are asymmetric around the mean, as we would expect because precisely because they should not include values outside of the [0,1] range:
...
Bootstrap is a powerful (and ultimately simple) method to obtain confidence intervals. One important limitation to keep in mind about the bootstrap though is that it is utterly dependent on the representativeness of the sample data. One direct consequence of that is that bootstrapped CIs become more robust the more sample data we have. If you're interested in learning more about bootstrap and sampling-based approaches to statistics, check out the [lecture on sampling-based approaches  ](https://github.com/tfjaeger/BCS206-207-R-lectures/tree/2024-PhD-skillz-workshop/scripts/R/5b-Sampling-based-approaches Extended.pdf).

# Confidence intervals for grouped (repeated measures) data

So far we have obtained CIs only for data from a single subject. When we have repeated measures from multiple subjects, we can still use the approaches discussed here. We should, however, apply them to the *by-subject means* rather than the raw data to obtain visualizations that approximate appropriately conservative (i.e., wide) CIs.^[Note that the approach described here is not ideal, as it has no notion of how much data there is for each subject (information loss is never ideal). There are hierarchical extensions of bootstrap that can take this information into account. Here we focus on the simpler approach of calculating CIs over by-subject averages. For what it is worth, this is also the approach that I use for the basic plots in basically every single paper I have been involved in. Generally, it is a good idea to identify the largest source of variability among the grouping factors in your data (e.g., for language research: subjects, items, language, genus, language famility, etc.), and to aggregate the data down to one observation per condition per level of the grouping variable with the largest variability. In psycholinguistic experiments, participants are *typically* the grouping variable with the largest variability (i.e., participants differ more from each other than items. However, this is not necessarily true for, e.g., corpus studies that explore a larger range of linguistic material.] 
