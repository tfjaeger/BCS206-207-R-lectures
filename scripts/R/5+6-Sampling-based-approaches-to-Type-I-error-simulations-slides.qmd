...
## Using simulations to estimate the statistical power of an analysis
...
## Using simulations to estimate *Type I error rate* of an analysis
...
2.  [We add a non-zero effect that we want to estimate power for (e.g., by subtracting half of it from one condition and adding half of it to the other condition).]{style="color:gray"}
2.  [We apply whatever type(s) of analysis we're interested in estimating **Type I error rate** for]{style="color: gray"}
3.  [We repeat this process many times, each time storing whether the p-value(s) of interest were less than our significance criterion.]{style="color: gray"}
4.  [The proportion of significant results is the **Type I error rate**.]{style="color:gray"}
